...
const AdminStoriesPage: React.FC = () => {
  ...
  const [storyType, setStoryType] = useState<'Text' | 'Comic'>('Comic');
  ...
  const CRAWL_TEMP_KEY = 'fimory_admin_story_crawl_temp';

  const getCrawledChapterKey = (chapter: ChapterMeta) =>
    chapter.url || `${chapter.slug}-${chapter.ordinal}`;
  ...
  const normalizeToStoragePath = (value: string) => {
    ...
    if (/^https?:\/\//i.test(normalized)) {
      try {
        ...
        // Nếu là URL đã nằm trên storage/uploads local thì convert về /storage
        if (parsed.pathname?.includes('/storage/') || parsed.pathname?.includes('/uploads/')) {
          normalized = parsed.pathname || normalized;
        } else {
          // Giữ nguyên URL bên ngoài để backend lưu thẳng CoverURL (fallback khi không tải local được)
          return normalized;
        }
      } catch {
        return normalized;
  ...
  const getPendingCrawlPaths = () => {
    const paths: string[] = [];
    if (chapterPreviewFolder && chapterPreviewFolder.startsWith('/storage/')) {
      paths.push(chapterPreviewFolder);
    }
    // Không auto-cleanup cover nữa để tránh xóa nhầm cover đã được gán vào truyện sau khi upload.
    return Array.from(new Set(paths));
  };

  const cleanupPendingCrawlAssets = async (paths?: string[]) => {
    if (!email) return;
    const targets = (paths ?? getPendingCrawlPaths()).filter((p) => p.startsWith('/storage/'));
    if (targets.length === 0) return;
    try {
      await cleanupCrawlTempAssets(email, { paths: targets });
    } catch (error) {
      console.warn('Không thể cleanup tạm dữ liệu crawl:', error);
    }
  };
  ...
  useEffect(() => {
    if (!email) return;
    const stale = sessionStorage.getItem(CRAWL_TEMP_KEY);
    if (!stale) return;
    try {
      const parsed = JSON.parse(stale);
      const paths = Array.isArray(parsed?.paths) ? parsed.paths.filter((p: string) => typeof p === 'string') : [];
      if (paths.length > 0) {
        cleanupPendingCrawlAssets(paths);
      }
    } catch {
      // ignore parse errors
    } finally {
  ...
  useEffect(() => {
    const paths = getPendingCrawlPaths();
    if (paths.length > 0) {
      sessionStorage.setItem(CRAWL_TEMP_KEY, JSON.stringify({ paths }));
    } else {
      sessionStorage.removeItem(CRAWL_TEMP_KEY);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chapterPreviewFolder, crawlCoverUrl]);
    ...
    const handleBeforeUnload = () => {
      ...
      try {
        if (navigator.sendBeacon) {
          navigator.sendBeacon('/api/crawl/cleanup-temp', new Blob([body], { type: 'application/json' }));
        }
      } catch {
        // ignore
      }
  ...
  const addNewChapter = async () => {
    ...
    try {
      const formData = new FormData();
      formData.append('title', newChapterTitle || `Chương ${editChapters.length + 1}`);
      ...
      if (!response.ok) {
        let message = 'Failed to add chapter';
        try {
          ...
          // ignore parse error for non-JSON response
        }
  ...
  const handleStoryUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ✅ Auto-create chapter nếu có ảnh chapter nhưng backend chưa trả chapterId.
        // Lưu ý: backend hiện chưa tự tạo chapter từ crawledImages trong API /admin/stories,
        // nên cần tạo chapter thủ công ở đây cho cả ảnh upload và ảnh crawl.
        if (createdChapterId) {
          ...
          // Auto-create chapter khi có dữ liệu ảnh chapter
          try {
            const chapterForm = new FormData();
            chapterForm.append('title', 'Chương 1');
        ...
        setUploadMessage(`✅ Upload thành công! Truyện đã được tự động duyệt với ${result.chapterCount || (storyType === 'Text' ? contentFiles.length : (chapterImages.length > 0 ? 1 : 0))} chương.`);
  ...
  const handleChapterCrawl = async () => {
    ...
    const targetChapter = crawledChapterList.find((ch) => getCrawledChapterKey(ch) === selectedCrawledChapterSlug);
    if (!targetChapter) {
    ...
    try {
      ...
      await cleanupPendingCrawlAssets();
                      ...
                      <div className="mt-3 space-y-3">
                          ...
                          <div className="flex items-center gap-2 w-full md:w-auto">
                            ...
                            >
                              ...
                              {crawledChapterList.map((chapter) => (
                                <option key={getCrawledChapterKey(chapter)} value={getCrawledChapterKey(chapter)}>
                        ...
                        <div className="max-h-48 overflow-y-auto border border-dashed border-gray-200 dark:border-gray-700 rounded p-2 bg-gray-50 dark:bg-gray-900/40">
                          {crawledChapterList.slice(0, 40).map((chapter) => (
                            <button
                              key={getCrawledChapterKey(chapter)}
                              onClick={() => {
                                setSelectedCrawledChapterSlug(getCrawledChapterKey(chapter));
                                setChapterCrawlMessage('');
                              }}
                              className={`w-full text-left px-2 py-1 rounded text-xs mb-1 ${
                                selectedCrawledChapterSlug === getCrawledChapterKey(chapter)
                                            ...
                                            <div className="flex items-center gap-2 flex-1">
                                              ...
                                              <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">
                                                {(() => {
                                                  ...
                                                  if (!t || lower === `chapter ${ch.ChapterNumber}` || lower === `${ch.ChapterNumber}`) {
                                                    return `Chương ${ch.ChapterNumber}`;
                                                  }
                                                  return t;
                                                })()}
                                              </span>
                                            ...
                                            <Button
                                              ...
                                              onClick={async () => {
                                                ...
                                                try {
                                                  ...
                                                  const response = await fetch(`/api/admin/stories/${editId}/chapters/crawl`, {
                                                    method: 'POST',
                                                    headers: {
                                                      'Content-Type': 'application/json',
                                                      'x-user-email': email,
                                                  ...
                                                  alert(`✅ Đã crawl ${result.imageCount ?? 0} ảnh và tạo chapter ${result.chapterNumber ?? nextChapterNum}`);
                                    ...
                                    <div className="space-y-2">
                                      <div>
                                        ...
                                        <Input
                                          value={newChapterTitle}
                                          onChange={(e) => setNewChapterTitle(e.target.value)}
                                          placeholder={`Chương ${editChapters.length + 1}`}
                                          className="text-xs h-7"
                                        />
                                      </div>
                                      {storyTypeForEdit === 'Text' ? (
