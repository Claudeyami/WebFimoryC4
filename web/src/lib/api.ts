...
export async function adminListMovies(email: string): Promise<any[]> {
  ...
  if (!res.ok) {
    console.error("Lỗi khi gọi API adminListMovies:", await res.text());
...
// FIX: Crawl movie từ URL (TMDB hoặc trang VN) - dùng POST như crawlStoryFromVN
...
export async function crawlStoryFromVN(
  ...
  const payload = await res.json();
  return (payload?.data ?? payload) as CrawlStoryResponse;
}
...
export async function crawlChapterImages(
  ...
  return (responsePayload?.data ?? responsePayload) as CrawlChapterResponse;
}

export async function cleanupCrawlTempAssets(
  email: string,
  body: {
  ...
  const res = await fetch(`${API_BASE}/crawl/cleanup-temp`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-user-email": email,
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || "Failed to cleanup crawled assets");
  }
  const payload = await res.json();
  return (payload?.data ?? payload) as { ok: boolean; deleted?: number; skipped?: number };
}
